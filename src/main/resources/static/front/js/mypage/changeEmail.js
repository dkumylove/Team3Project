            ...
            ajaxLoad("GET", `/api/member/email_dup_check?email=${email}`, null, "json")
                .then(data => {
                    if (data.success) { // 중복이메일인 경우
                        alert("중복된 이메일입니다.");
                        frmEmail.newEmail.focus();

                    } else { // 중복이메일이 아닌 경우
                        sendEmailVerify(email); // 이메일 인증 코드 전송
                        this.disabled = frmEmail.newEmail.readonly = true;
...
function callbackEmailVerifyCheck(data) {
    if (data && data.success) { // 인증 성공
        ...
        */
        ...
        // 3. 이메일 입력 항목 readonly 속성으로 변경
        frmEmail.newEmail.readonly = true;
...
const authCount = {
    ...
    start() {
        ...
        this.intervalId = setInterval(function() {
            ...
            if (authCount.count < 0) {
                ...
                emailVerifyEl.disabled = frmEmail.email.readonly = false;
                return;
            }
    ...
    */
    initialize() {
        ...
        emailVerifyEl.disabled = frmEmail.newEmail.readonly = true;
